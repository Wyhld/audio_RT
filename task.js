...
async function showInstructions() {
  instructions.draw();
  await win.flip();
  await util.waitForKeyPress(['space']);
}
...
async function runTrial(isSquare, withNoise = false) {
  ...
  trialClock.reset();
  const keys = await util.waitForKeyPress(['z', 'm', 'escape'], 2.0, trialClock);
...
async function showTestingInstructions() {
  const testInstr = new visual.TextStim({
    win,
    text:
